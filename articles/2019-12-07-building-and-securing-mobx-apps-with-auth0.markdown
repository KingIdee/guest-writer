...
State management is one of the major points of software applications and React apps are not exempted. It is one of the many functionalities that come packed with React by default. Little wonder React is even referred to as a *state management library* **by some. Because of how important state management is, a lot of solutions have sprung up in an attempt to make it easier. MobX is one of three popular options out there to help manage your state in React apps. Other options include [React Context API](https://reactjs.org/docs/context.html) and [Redux](https://redux.js.org/). 
...
- Reactions: MobX reactions are similar to computed values, but instead of producing a new value,  a reaction simply triggers a side effect, more like it performs a side operation. There are three types of reaction functions - `when`, `autorun`, and `reaction`.
